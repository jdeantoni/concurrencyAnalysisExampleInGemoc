...
public class PlsModelStateHelper implements IK3ModelStateHelper{
	...
	public K3ModelState getK3ModelState(EObject model) {
		...
		while (allContentIt.hasNext()) {
			...
			Class<?> clazz =null;
			clazz = K3DslHelper.getTarget(org.eclipse.gemoc.example.pls.rewritingrules.ContainerAspect.class);
	...
	public void restoreModelState(K3ModelState state) {
		for(ElementState elemState : state.getOwnedElementstates()) {
			for(Object o : elemState.getSavedRTDs()) {
				...
				try {
					if (n2v.value != null) {
						Method m = PlsRTDAccessor.class.getMethod("set"+n2v.name, EObject.class, n2v.value.getClass());
